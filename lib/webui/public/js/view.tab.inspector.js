var inspectorHandler = {
    ...
    inspect: function(record) {
        ...
        var url = record.data.req.method + ' HTTP/' + record.data.req.httpVersion + ' ' + record.data.url + '\n';
        ...
        var getforms = [],
            postforms = [];
        var getQueryString = record.data.url.split('?')[1] || '';
        if (getQueryString) {
            getforms = ([{
                'name': '== GET =='
            }]).concat(utils.parseQueryString(getQueryString).arr);
        }
        if (record.data.reqBody) {
            try {
                var postheaders = [];
                ...
                postforms = postheaders.concat(utils.parseQueryString(record.data.reqBody).arr);
            } catch (e) {
                $cmp.logWin.setValue('[' + $cmp.editIdx + '] POST not standand urlencoded data.' + '\n' + $cmp.logWin.getValue());
            }
        }
        reqWebformStore.loadData(getforms.concat(postforms));

        // res
        var resHeadersText = utils.objToPlain(record.data.resHeaders);
        var resRaw = $cmp.inspactors_res_raw;
        var status = 'HTTP/' + record.data.req.httpVersion + ' ' + record.data.result + '\n';
        var reqBody = record.data.reqBody ? ('\n' + record.data.reqBody) : '';
        resRaw.setValue(status + resHeadersText + reqBody);
        var headers = utils.parseHeader(resHeadersText);
        resHeaderStore.loadData(headers);
        var cookies = utils.filterCookieHeader(record.data.resHeaders);
        resCookieStore.loadData(cookies);
...
var RES_VIEWER = {
    ...
    showJsonViewer: function(el, record) {
        // features: paste text -> json view
        var textValue = '';
        if (RES_VIEWER._type == 'Text') {
            textValue = RES_VIEWER._el.getValue();
        }
        ...
        if (textValue) {
            ...
            Ext.Ajax.request({
                ...
                success: function(response, options) {
                    setJsonView(response.responseText);
                },
...
var TAB_INSPECTORS_REQ = {
    ...
    items: [{
        ...
        layout: 'fit',
        items: [{
            id: 'inspactors_req_webforms',
            xtype: 'gridpanel',
            ui: 'blue-panel',
            store: reqWebformStore,
            columns: [{
                text: 'header',
                dataIndex: 'name',
                menuDisabled: true,
