var observers = {
    ...
    onRemoveImages: function() {
        ...
        // Poor performance on Ext.data.store.remove
        // http://www.sencha.com/forum/showthread.php?215971-Poor-performance-on-Ext.data.store.remove(records)
        store.suspendEvents();
        store.remove(records);
        store.resumeEvents();
        $cmp.sessionGrid.getView().refresh();
    },
    ...
    onSpeedLimitItemClick: function(item, e) {
        $cmp.data.speedLimit = item.text;
        observers.onMenuSet('speedLimit', item.text);
    },
